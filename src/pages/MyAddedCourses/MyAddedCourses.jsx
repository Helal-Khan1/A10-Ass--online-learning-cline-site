...
const MyAddedCourses = () => {
  ...
  const detatehandalar = (id) => {
    instance.delete(`/all_courses/${id}`).then((res) => {
      const detatecous = myaddCours.filter((cors) => cors._id !== id);
      toast("Successfully delete ");
      setMyaddCours(detatecous);
  ...
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-10 justify-items-center p-5 ">
        {myaddCours.map((course, index) => (
          <motion.div
            key={course._id}
          ...
          >
            ...
            <div className="p-5 flex flex-col justify-between flex-grow">
              ...
              <button
                onClick={() => detatehandalar(course._id)}
                className="bg-red-400  text-white px-5 py-1 rounded-full text-sm flex items-center gap-2 hover:bg-red-500 mt-5 transition-all font-bold "
              >
                Delate
              </button>
            </div>
